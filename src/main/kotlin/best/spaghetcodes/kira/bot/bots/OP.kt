...
class OP : BotBase("/play duels_op_duel"), Bow, Rod, MovePriority, Potion, Gap, Flint {
    ...
    // ---- 2e Speed / 2e Regen : cast aux PIEDS (fiabilisé, double tentative) ----
    private fun hasEffect(nameSubstr: String): Boolean {
        val p = mc.thePlayer ?: return false
        for (effect in p.activePotionEffects) {
            val nm = effect.effectName.lowercase()
            if (nm.contains(nameSubstr)) return true
        }
        return false
    }
    ...
    private fun feetSplash(damage: Int, onComplete: (() -> Unit)? = null) {
        ...
        waitUntilOnGround(maxWaitMs = 420) {
            ...
            attemptOnce("try#1") {
                    ...
                    if (!ok) {
                        ...
                        attemptOnce("try#2") {
                            TimeUtils.setTimeout({
                                ...
                                Mouse.startTracking()
                                onComplete?.invoke()
                            }, RandomUtils.randomIntInRange(280, 360))
                        }
                    } else {
                        takingPotion = false
                        Mouse.startTracking()
                ...
                }, RandomUtils.randomIntInRange(300, 380))
            }
        }
    }
    ...
    private fun performPreGapActionStrict(initialDistance: Float, onComplete: () -> Unit) {
        ...
        if (initialDistance <= 3.5f) {
            ...
            if (canUseFlint) {
                useFlint(initialDistance) { onComplete() }
                return
            }
    ...
    override fun onTick() {
        if (opponent() != null && mc.theWorld != null && mc.thePlayer != null) {
            ...
            // =====================  SOINS (seuils neutres + garde distance) =====================
            val recentRegen = now - lastRegenUse < 30_000L
            val healthOnly = p.health
